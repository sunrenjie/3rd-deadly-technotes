...
#define PPN(la)		(((uintptr_t) (la)) >> PTXSHIFT) // <-------------------- PTXSHIFT can be defined later
#define VPN(la)		PPN(la)		// used to index into vpt[]

// page directory index
#define PDX(la)		((((uintptr_t) (la)) >> PDXSHIFT) & 0x3FF) // <---------- 0x3ff: 10 bits of '1'
...
#define PTSIZE		(PGSIZE*NPTENTRIES) // bytes mapped by a page directory entry <- 4M
 ...
 *                     |                              | RW/--
 *    KERNBASE ----->  +------------------------------+ 0xf0000000 <------------- 0xffffffff - 0xf0000000 + 0x1 = 0x100000000 = 256MB
 *                     |  Cur. Page Table (Kern. RW)  | RW/--  PTSIZE
 *    VPT,KSTACKTOP--> +------------------------------+ 0xefc00000      --+
 *                     |         Kernel Stack         | RW/--  KSTKSIZE   |
 *                     | - - - - - - - - - - - - - - -|                 PTSIZE
 *                     |      Invalid Memory (*)      | --/--             |
 *    ULIM     ------> +------------------------------+ 0xef800000      --+ <---- 0xffffffff - 0xef800000 = 263MB (kernel va size)
...
// Virtual page table.  Entry PDX[VPT] in the PD contains a pointer to <--------- meaning the 1st entry in this vpt points to itself?
